...
nw:set-context nodes links
 nw:save-graphml "b6a_117_1.graphml"
...
@#$#@#$#@
## WHAT IS IT?
This model simulate transition in the construction state of a house as a function of interactions within the social structure composed by the households located into the boundaries of a housing block. Social structure is replicated as a social network where the influence of houses in a greater state is transmitted using a epidemic diffusion mechanism through the space of housing block represented as a regular lattice. Model adapts Netlogo Virus on a Network model by Stonedahl and Wilensky (2008), within a spatial setting that represents a housing block.


## HOW IT WORKS

Hypothesized Influence operates through a model of epidemics diffusion called SIRS (Susceptible-Infected-Resistant-Susceptible). Analogous to first three states for the case of consolidation are the following agent states: Susceptible, Upgraded and Delayed. Diffusion algorithm operates into the initial settings of the network through discrete time steps. Nodes correspond to the households and non-directed edges are used to represent their relations into the network, which is assumed to have a random structure. Upgraded nodes are those agents that have the highest state witin the block and therefore exert influence on others in the neighborhood (network). Susceptible agents respond immediately to the influence of connected upgraded neighbors; once upgraded these agents become Delayed and still connected with initial associates can react to influences but its upgrade decision is related to an arbitrary probability that represents the level of affordability or resources availability to upgrade, which is an actual cause of delay in the process of upgrade the state.


## HOW TO USE IT

Initial settings consider actual composition and configuration of blocks idealizaed as a lattice where each cell correspond to a house in its particular state at the begining of the simulation.
Sliders "block-size", "number-neighbors", "time-steps" and "max_number_D" arte related to initial characteristics such as number of houses in the block, number of non-directed links each node have, and two stop conditions which are selected according to which one occurs first: a determined number of time-steps or reach a maximum number of cells in state D. Block spatial configuration of states is uploaded via a csv file that represents four consolidation states A, B, C and D as numbers 0, 1, 2, 3 respectively.


## THINGS TO NOTICE

There are three main parameters that govern simulation: "upgrade-spread-chance", "recovery-chance" and "gain-resistance-chance"; these correspond to the probabilities to upgrade (change construction state), getting resources to upgrade (afford change), and delay upgrade due to lack of resources.
## THINGS TO TRY
